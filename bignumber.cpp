...
namespace mc {
    ...
    BigNumber::BigNumber(uint64_t p) : digits(ToString(p)) {
    }
    ...
    BigNumber::BigNumber(const string& p) : digits(normalize(p)) {
    }

    const BigNumber& BigNumber::operator=(const BigNumber& p) {
        digits = p.digits;
        return *this;
    }

    const BigNumber& BigNumber::operator=(const string& p) {
        digits = normalize(p);
        return *this;
    }

    const BigNumber& BigNumber::operator=(const uint64_t& p) {
        digits = ToString(p);
        return *this;
    }
    ...
    size_t BigNumber::NrOfDigits() const {
        return digits.size();
    }
